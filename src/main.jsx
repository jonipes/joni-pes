...
const games = [
  ...
  { id: 'pubg', title: 'PUBG Mobile', studio: 'Tencent Games', img: '/assets/pubg.png' },
    ...
    <div className="min-h-screen flex flex-col">
      <header className="px-4 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full bg-white/10 flex items-center justify-center glow">
        ...
        <nav className="hidden md:flex gap-6">
          ...
          <a className="hover:underline">Account Store</a>
...
function NavItem({label, icon}){
  return (
    <button className="flex flex-col items-center gap-1 text-xs text-white/90 p-2 w-20">
...
function GameCard({game}){
  const open = () => {
    // open new tab to a placeholder page (replace with real links later)
    window.open(`/game/${game.id}`,'_blank')
  }

  return (
    <div className="bg-white/6 rounded-xl p-0 overflow-hidden group cursor-pointer" onClick={open}>
      <div className="relative bg-gradient-to-b from-white/6 to-white/8 p-4 game-card">
        <div className="w-full h-28 flex items-center justify-center bg-gradient-to-br from-white/3 to-transparent rounded-lg">
          <img src={game.img} alt={game.title} className="w-20 h-20 object-contain transform transition-transform group-hover:scale-105" />
        </div>
      </div>
      <div className="p-4">
